...
fprintf(dataFile, ['Cost   ($)  Length (m)  Friction  Diam. (m)',...
        '  Bend 1  Bend 2  Bend 3  Bend 4  Head Loss\n']);
...
rate = input('Enter flow rate in m3/s:');
...
for col = 1:(columnsPipes - 1)
    for row = 2:rowsPipes
        ...
        zoneData(n, 5) = bendCoeffOne;
        zoneData(n, 6) = bendCoeffTwo;
        zoneData(n, 7) = bendCoeffThree;
        zoneData(n, 8) = bendCoeffFour;
        zoneData(n, 9) = hLoss;
        
        fprintf(dataFile, ['%10.2f    %8.2f      %.2f       %.2f',...
                '    %.2f    %.2f    %.2f    %.2f    %.2f\n'],... 
                totalCost, length, friction, diameter, bendCoeffOne,...
